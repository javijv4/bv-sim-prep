...
def dict_to_pfile(out_name, dictionary):    # TODO make this way more general
    f = open(out_name, "w")
    for key in dictionary.keys():
        f.write('#' + key + '=' + str(dictionary[key]) + '\n')
    f.close()

def pfile_to_dict(pfile, fmt=float):    # TODO make this way more general
    dictionary = {}
    f = open(pfile, "r")
    for line in f:
        # Check for comments
